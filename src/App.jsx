      ...
      <main className="do-main">
        ...
        <div className="do-page">
          {view==="dashboard"&&<div>
            ...
            <div className="do-tbl"><div className="do-tbl-hd"><h3>Proximas Citas</h3></div>{citasSem.length>0?<table><thead><tr><th>Paciente</th><th>Fecha</th><th>Hora</th><th>Tipo</th><th>Estado</th><th></th></tr></thead><tbody>{citasSem.map((c,i)=>{const p=pacs.find(x=>x.id===c.pacienteId);return <tr key={c.id} onClick={()=>p&&setSelPat(p)}><td><div className="do-pcell"><Av name={c.paciente} i={i}/><span className="do-pname">{c.paciente}</span></div></td><td>{fmtD(c.fecha)}</td><td>{c.hora}</td><td>{c.tipo}</td><td><Tag type={c.estado}/></td><td>{p&&<WA phone={p.telefono} msg={"Recordatorio cita "+fmtD(c.fecha)+" "+c.hora}/>}</td></tr>})}</tbody></table>:<div className="do-empty"><h4>Sin citas esta semana</h4></div>}</div>
          </div>}
          {view==="pacientes"&&<div className="do-tbl"><div className="do-tbl-hd"><h3>Pacientes ({filtP.length})</h3><div className="do-filters">{["Todos","Nuevo","Recurrente","Convenio"].map(f=><Chip key={f} label={f} active={pF===f} onClick={()=>setPF(f)}/>)}</div></div><table><thead><tr><th>Paciente</th><th>Telefono</th><th>Tipo</th><th>Ultima</th><th>Proxima</th><th>Fuente</th><th></th></tr></thead><tbody>{filtP.map((p,i)=><tr key={p.id} onClick={()=>setSelPat(p)}><td><div className="do-pcell"><Av name={p.nombre} i={i}/><div><div className="do-pname">{p.nombre}</div><div className="do-pdetail">{p.email||"Sin email"}</div></div></div></td><td>{p.telefono}</td><td><Tag type={p.tipo}/></td><td>{fmtD(p.ultimaVisita)}</td><td style={{color:p.proximaCita?"#2A7C6F":"#D4726A"}}>{p.proximaCita?fmtD(p.proximaCita):"Sin agendar"}</td><td style={{fontSize:12,color:"#C4B5A0"}}>{p.fuente}</td><td><WA phone={p.telefono} msg={"Hola "+p.nombre.split(" ")[0]+", le escribimos de Diane Opticas."}/></td></tr>)}</tbody></table>{filtP.length===0&&<div className="do-empty"><h4>Sin resultados</h4></div>}</div>}
...
@media(max-width:768px){.do-side{transform:translateX(-100%)}.do-side.open{transform:translateX(0)}.do-main{margin-left:0}.do-stats{grid-template-columns:1fr}.do-top{padding:10px 14px}.do-page{padding:14px}.do-search{width:100%;order:3}.do-mob-tog{display:flex !important}.do-ficha{width:100%}}
